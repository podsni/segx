...
while [[ "${1:-}" =~ ^- ]]; do
  case "$1" in
    ...
    --add-user) ADD_USER=1; ADD_USER_SET=1 ;;
    --no-add-user) ADD_USER=0; ADD_USER_SET=1 ;;
...
Usage: $0 [options]
  -y, --yes                Non-interactive (jawab "ya" otomatis)
      ...
      --no-add-user        Jangan ubah keanggotaan grup docker
      --no-daemon-json     Jangan tulis /etc/docker/daemon.json
Env:
  AUTO_YES=1, ADD_USER=1|0, CONFIGURE_DAEMON=0 (sama seperti flags)
EOF
      exit 0
      ;;
...
# Auto aktifkan penambahan user jika terdeteksi menjalankan melalui sudo
if [ "$ADD_USER_SET" -eq 0 ]; then
  LOGIN_USER="$(resolve_login_user 2>/dev/null || true)"
...
ensure_docker_group() {
  if getent group docker >/dev/null 2>&1; then
    log "Grup 'docker' sudah tersedia."
  else
    log "Membuat grup 'docker'."
    groupadd docker
  fi
}
...
maybe_add_user() {
  ...
  if [ "$do_add" -eq 1 ]; then
    ...
    CURRENT_USER="$(resolve_login_user 2>/dev/null || true)"
    ...
    if [ "$CURRENT_USER" = "root" ]; then
      warn "User aktif adalah root; lewati penempatan grup docker."
      return
    fi

    if id -nG "$CURRENT_USER" | tr ' ' '\n' | grep -qx docker; then
      log "User '$CURRENT_USER' sudah merupakan anggota grup docker."
    else
      usermod -aG docker "$CURRENT_USER"
      log "User '$CURRENT_USER' ditambahkan ke grup docker."
      warn "Logout/login atau jalankan 'newgrp docker' agar perubahan grup aktif."
    fi
  fi
}
